...
const Start = ({ onStart }) => {
  ...
  const onHandlerConfirm = () => {
    ...
    if (isNaN(chosenDni) || chosenDni <= 1000000 || chosenDni > 99999999) {
      Alert.alert('Invalid DNI', 'DNI has to be a number between 1000000 and 99999999', [
        { text: 'Okay', style: 'destructive', onPress: () => setDniOption('') },
      ]);
    } else {
      setConfirmed(true);
  ...
  const Confirmed = () =>
    confirmed ? (
      <Card style={styles.confirmedContainer}>
        ...
        <Button title="Start" onPress={onHandlerStart} color={theme.colors.primary} />
      </Card>
    ) : null;
